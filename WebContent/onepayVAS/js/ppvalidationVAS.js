...
$(document).ready(function() {
	   $('#bank_id li').click(function() {
		...
		$('.atombankName').val(value_li);
		 $.ajax({
		     type:"POST",    
		     	url: "/pppayfiVAS/ajaxAction?type=getemidetails",
...
jQuery(document).ready(function()
				...
				{	
					...
					// alert("Six digit :: "+cardNumber);
								...
								if ( hdfcBinType =="ONUS_CREDIT" || hdfcBinType == "ONUS_DEBIT" )
								{
									...
									({				
										type: 'POST',
										url: "/pppayfiVAS/ajaxAction?type=getONUSBINDetail",
										async: false,
								...
								else if(hdfcBinType =="Other_Card")
								{
									...
									({				
										type: 'POST',
										url: "/pppayfiVAS/ajaxAction?type=getONUSBINDetailOtherCard",
										async: false,
										data: {cardNumber : cardNumber , hdfcBinType : hdfcBinType,merchantId:merId},
										success: function(data)
										{	
 ...
 // alert("myCardNo >>>>> in ValidateCard method >>>>>> "+myCardNo);
	  ...
	  else if(myCardNo.startsWith('601') || myCardNo.startsWith('622') || myCardNo.startsWith('64'))
	  {
	 	myCardType='Discover';
	...
	//  $('#emailId').focus();
        ...
        return false;
    		...
    		({				
    			...
    			url: "/pppayfiVAS/ajaxAction?type=validateOtp",
    			async:false,
    			success: function(data)
    			{
				   $(".red-btn").css( "pointer-events", "auto" );
 ...
 function sendOtp(cmobileNo, cemailId, txnId, merchantName)
    {
    	...
    	//var jsessionId = '${cookie.JSESSIONID.value}';
    		...
    		({				
    			...
    			url: "/pppayfiVAS/ajaxAction?type=sendOtp",
...
function storedcardFill(cardno){
	...
	//  document.getElementById("phone").value = cMob;
		...
		({				
			...
			url: "/pppayfiVAS/ajaxAction?type=decryptCard",
			success: function(result)
			{
				// alert("Result ::: "+result);
	  ...
	  if (isConfirm) {
	    $.ajax
		    	({				
		    		...
		    		url: "/pppayfiVAS/ajaxAction?type=removeCard",
		    		success: function(result)
		    		{
		    			$("#li"+value_i).remove()
...
$('#pay').click(function (){
	...
	$.ajax
 		({				
 			...
 			url: "/pppayfiVAS/UpiCheckServlet",
 			success: function(result)
 			{
 				console.log(result);
...
if (i !== -1)
{
	...
	({				
		...
		url: "/pppayfiVAS/ajaxAction?type=getStatusBQRTransaction",
...
function getSurcharge()
{
	...
	({				
		...
		url: "/pppayfiVAS/SurchargeCalculation",
