...
<%
	...
	*/
	...
	we suggest merchant SHOULD take this values from secure channel*/
	  Logger log = LoggerFactory.getLogger("JSPS.FSSTranVbyV.jsp");
	...
	if (request.getParameter("MD") != null) 
	{
		...
		if(TM.getRespStatus().equalsIgnoreCase("5"))
		{
			...
			log.info("TransVbyV.jsp ::: second response TransVbyV.jsp ::: for txnid=" + Mid + " bankId : " + bankId
					+ " :: Mid : " + Mid + "  instrumentId ::: :: " + instrumentId);
		}
	...
	if (args.length > 1 && MTrackid.equals(args[1])) {
		log.info("TranVby.jsp ::: for id="+MTrackid+" There is no tempered ... txn id is match...instrument" + instrumentId);
		cardno = args[0];
	...
	if (spMidKey != null && spMidKey.size() > 0) {
		...
		log.info("FSSFSSTranVbyV.jsp :::for txnid="+MTrackid+"  Attributes set in Session :: SPMID : "
	...
	log.info("FSSFSSTranVbyV.jsp ::: for txnid="+MTrackid+" Fetch Attributes from Session :: SPMID : "
			+ session.getAttribute("spMid") + " And SPKEY : " + session.getAttribute("spKey"));
		}
	} catch (Exception e) {
		log.error("FSSFSSTranVbyV.jsp ::: for txnid="+MTrackid+" Error in fetching SPMID and SPKEY :: ", e);
	}

	log.info("FSSFSSTranVbyV.jsp :::for txnid="+MTrackid+"  SPMID : " + spMid + " :: SPKEY : " + spKey);
	...
	log.info("for txnid="+MTrackid+" -------------LOG FOR HDFC FSSTranVbyV.jsp --------------");
	...
	*/
	...
	try {
		...
		if (PARes == null) {
			...
			if (!bankId.equalsIgnoreCase("Rupay")) {
				Envreq = TranportalID + TranportalPwd + strcard + strcvv + strexpyear + strexpmonth + straction
						+ stramt + strcurrency + strmember + strtrackid + strinitudf1 + strinitudf2
						+ strinitudf3 + strinitudf4 + strinitudf5;
			} else //For Rupay
			{

				String responseURL = "<merchantResponseUrl>"
						+ PGUtils.getPropertyValue("FSSmerchantResponseUrlRupay") + "</merchantResponseUrl>";
				String errorURL = "<merchantErrorUrl>" + PGUtils.getPropertyValue("FSSmerchantResponseUrlRupay")
						+ "</merchantErrorUrl>";

			
				Envreq = TranportalID + TranportalPwd + strcard + strcvv + strexpyear + strexpmonth + straction
						+ stramt + strcurrency + strmember + strtrackid + strinitudf1 + strinitudf2
						+ strinitudf3 + strinitudf4 + strinitudf5 + responseURL + errorURL;
			...
			String sURL = PGUtils.getPropertyValue("fssmpiVerifyEnrollmentUrlHDFC");
...
<BODY OnLoad="OnLoadEvent();">
	<form name="form1" action="<%=acsurl%>" method="post">
		...
		<%
			String termURL = PGUtils.getPropertyValue("fsstermUrlHDFC");
		%>
...
<%
	/*************Enrolled card condition Ends here************/
					...
					String NEAuthReq = TranportalID + TranportalPwd + strcard + strcvv + strexpyear
							+ strexpmonth + straction + stramt + strcurrency + strmember + strtrackid
							+ strinitudf1 + strinitudf2 + strinitudf3 + strinitudf4 + strinitudf5 + strZIP
							+ strADDR + strECIValueTag;

					/* Below URL is used only when NOT ENROLLED response is received from Payment Gateway */
					/* This is test environment URL,production URL will be different and will be shared by Bank during production movement */
									
					String sURL2=PGUtils.getPropertyValue("fsstranPortalUrlHDFC");
					...
					*/
					...
					log.info("FSSTranVbyV.jsp ::: for txnid="+MTrackid+" NOT ENROLLED Response :: NEResResult : " + NEResResult);
					...
					JSONObject errorCodeJSON = DataManager.getErrorCode(spErrorCode,spId);
					...
					if (!TM.getRespStatus().equalsIgnoreCase("1")) {
						...
						TM.setModified_By("FSSRespHandler");
						TM.setModified_On(resp_date_Time);
						dm.updateTxn(TM);

					
						request.setAttribute("ResTrackId", NEResTrackId);
						request.getRequestDispatcher("/respHandlerFSS").include(request, response);

					} else {
						log.info(
								"FSSTranVbyV.jsp ::: for txnid="+MTrackid+" Transaction is already Processed.Response already updated for Txn Id '"
										+ TM.getId() + "'.");
				...
				// changes 26-04-2019 Rupay Card Change on else If block
					...
					log.info("FSSFSSTranVbyV.jsp : for txnid="+MTrackid+"  paymentPage : " + paymentPage);
					log.info("FSSFSSTranVbyV.jsp : for txnid="+MTrackid+" paymentid : " + paymentid);
%>
<HTML>
<BODY OnLoad="OnLoadEvent();">
...
<%
					...
					*/
					...
					log.info(
							"FSSTranVbyV.jsp ::: for txnid="+MTrackid+" Response is neither ENROLLED nor NOT ENROLLED :: strEnrollmentResult : "
									+ InitialResponse);
				...
				*/
				...
				log.info("FSSTranVbyV.jsp ::: for txnid="+MTrackid+" ERROR IN INITIAL REQUEST PROCESSING :: strError : " + strError);
				...
				log.info("FSSTranVbyV.jsp :::for txnid="+MTrackid+"  SP Error Code :: spErrorCode : " + spErrorCode);
			...
			log.info("for authid="+PaymentIDMD+"  ============== Response Log PARes != Null Then ============== ");
			...
			if (ENMPIAuthResponse.length() > 0) {
				...
				*/
				...
				/*Redirecting to Final Status Page with required parameters*/
				log.info("FSSTranVbyV.jsp :: Test :: txn_id : EnMPIResTrackId  " + EnMPIResTrackId
				...
				log.info(new SimpleDateFormat().format(new Date())+ " for txnid = "+EnMPIResTrackId+" TM.getRespStatus >>>>>>>>>>>> +++++++++++ " + TM.getRespStatus());

				if (null == EnMPIReserror || EnMPIReserror == "") {
					EnMPIReserror = "NA";
					log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())
							+ "for txnid = "+EnMPIResTrackId+" EnMPIReserror ---------------------------------- " + EnMPIReserror);
				}

				if (EnMPIReserror.equalsIgnoreCase("!ERROR!-CM90004-Duplicate Record")) {

					log.info("for txnid = "+EnMPIResTrackId+" response not send or updated for error=" + EnMPIReserror + " for service_auth_id="+ EnMPIResPayid);
				} else {
					...
					if (!TM.getRespStatus().equalsIgnoreCase("1")) {
						...
						if ((EnMPIReserror != null || EnMPIReserror != "")
							...
							enqReq = TranportalID + TranportalPwd + "<action>8</action>" + "<transid>"
									+ EnMPIResTranid + "</transid>" + "<trackid>" + EnMPIResTrackId
									+ "</trackid>" + "<amt>" + addedAmt + "</amt>" + strcurrency
									+ "<member>Enquiry Name</member><udf5>TrackID</udf5>";
						} else {
						...
						log.info("for txnid = "+EnMPIResTrackId+" FSSTranVbyV.jsp ::: Verification/Enquiry Response Result :: enqResResult : "
								+ enqResResult);
						...
						if (responseCode.equalsIgnoreCase("CAPTURED") && enqResResult.equalsIgnoreCase("SUCCESS")) {
							log.info("for txnid = "+EnMPIResTrackId+" respHandlerFSS ::: ===================Success =====================");
							responseCode = "Ok";
							errorCode = "00000";
							...
							log.info("for txnid = "+EnMPIResTrackId+" FSSTranVbyV.jsp ::: NOT CAPTURED :: EnMPIResResult : " + responseCode
									+ " :: EnMPIReserror : " + EnMPIReserror);
						    ...
						    //String spErrorCode = PGUtils.GetTextBetweenTags(NEResResult, "-", "-");
								...
								spErrorCode = EnMPIReserror.substring(EnMPIReserror.indexOf("-") + 1,
										EnMPIReserror.lastIndexOf("-"));

							JSONObject errorCodeJSON = DataManager.getErrorCode(spErrorCode,spId);//PGUtils.getPropertyValue("spIdHDFC"));
							...
							log.info("for txnid = "+EnMPIResTrackId+" FSSTranVbyV.jsp ::: ERROR while fetching Enquiry Response :: enqResResult : "
									+ enqResResult);
						...
						log.info(
								"FSSTranVbyV.jsp ::: for txnid = "+EnMPIResTrackId+" Transaction is already Processed.Response already updated for Txn Id '"
										+ TM.getId() + "'.");
						request.setAttribute("errorMsg",
								"Error 10054 : Transaction is already Processed.Response already updated for Txn Id '"
										+ TM.getId() + "'.");
				...
				*/
				...
				if (!TM.getRespStatus().equalsIgnoreCase("1")) {
					...
					log.info(
							"FSSTranVbyV.jsp ::: for txnid = "+TM.getId()+" Transaction is already Processed.Response already updated for Txn Id '"
									+ TM.getId() + "'.");
					request.setAttribute("errorMsg",
							"Error 10054 : Transaction is already Processed.Response already updated for Txn Id '"
									+ TM.getId() + "'.");
		...
		log.info("============== LOG END FOR HDFC FSSTranVbyV.jsp ==============");
	} catch (Exception e) {
		...
		/*Redirecting to Final Status Page with required parameters along with Error*/
		 TM = dm.getTxnMaster(MTrackid);
		log.error("for txnid="+MTrackid+" FSSTranVbyV.jsp ::: Error Occurred :: ", e);
		...
		if (!TM.getRespStatus().equalsIgnoreCase("1")) {
			...
			log.info("FSSTranVbyV.jsp ::: for txnid="+MTrackid+" Transaction is already Processed.Response already updated for Txn Id '"
					+ TM.getId() + "'.");
			request.setAttribute("errorMsg",
					"Error 10054 : Transaction is already Processed.Response already updated for Txn Id '"
							+ TM.getId() + "'.");
		...
		log.info("TranVbyV.jsp ::: for txnid=" + MTrackid+ " Transaction is already posted to TransVbyV with status 5 " + TM.getId() + "'.");
		request.setAttribute("errorMsg","Error 10060 : Transaction is already Transaction is alreday posted to TransVbyV '" + TM.getId()+ "'.");
		request.getRequestDispatcher("txnValidationErrors.jsp").include(request, response);
	}
%>
