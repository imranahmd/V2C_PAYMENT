...
<%
	...
	String encRespXML = null;
	  Logger log = LoggerFactory.getLogger("JSPS.FSSRedirect.jsp");
		...
		*/
		if (TM.getRespStatus().equalsIgnoreCase("5")) 
		{
			...
			if (ErrorNo == null) {
				...
				String verificationurl = PGUtils.getPropertyValue("fsstranPortalUrlHDFC"); //"https://securepayments.fssnet.co.in/hdfcbank/servlet/TranPortalXMLServlet";
				...
				log.info("for pg_id=" + TM.getId() + " paymentid=" + paymentid
						...
						+ "spIdFSSRupay:::::" + PGUtils.getPropertyValue("spIdFSSRupay"));

				spMidKey = new DataManager().getSPMidKey(TM.getMerchantId(), TM.getBankId(),
						TM.getInstrumentId(), PGUtils.getPropertyValue("spIdFSSRupay"));
				...
				if (!TM.getRespStatus().equalsIgnoreCase("1")) {
					...
					TM.setModified_By("FSSRespHandlerRupay");
					...
					log.info(
							"FSSRedirect.jsp ::: Transaction is already Processed.Response already updated for Txn Id '"
									+ TM.getId() + "'.");
				...
				//verification
				try {
					String verificationurl = PGUtils.getPropertyValue("fsstranPortalUrlHDFC");
				...
				JSONObject errorCodeJSON = DataManager.getErrorCode(ErrorNo,
						PGUtils.getPropertyValue("spIdFSS"));
...
<%!//=======This is GetTextBetweenTags function which return the value between two XML tags or two string =====
	...
	//Logger log = log.getLogger("JSPS.FSSRedirect.jsp");
