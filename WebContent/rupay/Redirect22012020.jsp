...
<%	
	...
	*/
		...
		{	
			...
			/* String verificationurl="https://securepgtest.fssnet.co.in/ipayb/servlet/TranPortalXMLServlet"; */
			
			String verificationurl=PGUtils.getPropertyValue("tranPortalUrlHDFC");//"https://securepayments.fssnet.co.in/hdfcbank/servlet/TranPortalXMLServlet";
			...
			log.info("getSPMidKey()  parameters::::::::::MerchantId:::::"+TM.getMerchantId()+"bankId::::::::::"+TM.getBankId()+"InstrumentId::::::::"+TM.getInstrumentId()+"spIdHDFCRupay:::::"+PGUtils.getPropertyValue("spIdHDFCRupay"));
			
			spMidKey = new DataManager().getSPMidKey(TM.getMerchantId(),TM.getBankId(),TM.getInstrumentId(),PGUtils.getPropertyValue("spIdHDFCRupay"));
			
			String sVerifyString ="<member>Anil</member>"
				+ "<action>8</action>"
				+ "<amt>"+amt+"</amt>"
				+ "<trackid>"+trackid+"</trackid>"
				+ "<transid>"+tranid+"</transid>"
				+ "<udf5>"+trackid+"</udf5>"
				+ "<ID>"+spMidKey.get(0).toString()+"</ID>"
				+ "<password>"+spMidKey.get(1).toString()+"</password>";	
				
				log.info("Verification String ::::::sVerifyString"+sVerifyString);
				...
				String NEResTranid=GetTextBetweenTags(InitialResponse,"<tranid>","</tranid>");//It will give Transaction ID
				 ...
				 log.info("NEResResult   	 ==>    "+NEResResult);
				...
				{
					log.info("respHandlerHDFC ::: ===================Success =====================");
					responseCode="Ok"; 
					errorCode = "00000";
				...
				}else
				{
					responseCode="F";	
					message = "Failed";
					log.info("Redirect.jsp ::: NOT CAPTURED :: responseCode : "+responseCode+" :: message : "+message);
				...
				{
					log.info("TranVbyV.jsp ::: Transaction is already Processed.Response already updated for Txn Id '"+TM.getId()+"'.");				
		...
		{	
			...
			JSONObject errorCodeJSON =  DataManager.getErrorCode(ErrorNo, PGUtils.getPropertyValue("spIdHDFC"));
...
<body onload="redirectRequest();">
	...
	<form name="RedirectForm" action="<%=TM.getReturn_url()%>" method="post">
...
<!-- 			response.sendRedirect(+"?merchantId="+TM.getMerchantId()+"&respData="+encRespXML); 
 -->			
 			<tr>
				<td><input type="hidden" name="respData" value="<%=encRespXML%>"></td>
			</tr>
			<tr>
				<td><input type="hidden" name="merchantId" value="<%=TM.getMerchantId()%>"></td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
			</tr>
